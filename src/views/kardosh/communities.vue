<template>
  ...
  <section class="relative z-10 -mt-8 lg:-mt-10">
    <div class="container-fluid">
      <ul
        class="communities-stats grid grid-cols-2 lg:grid-cols-4 gap-4 list-none p-0 m-0 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-md px-4 py-6 md:px-8"
      >
        <li v-for="stat in PAGE_STATS" :key="stat.label" class="text-center px-2">
          <p class="text-2xl md:text-3xl font-semibold text-slate-900 dark:text-white tabular-nums">{{ stat.value }}</p>
